...
impl UtxoStore {
    fn new() -> Self {
        UtxoStore {
            my_keys: Vec::new(),
            utxos: Arc::new(SkipMap::new()), //SkipMap is the cocurrent alternative to the binary tree map, in the same way that the DashMap is the cocurrency alternative of the Hashmap
        }
    }
...
impl Core {
    ...
    pub fn load(config_path: PathBug) -> Result<Self> {
        ...
        for key in &config.my_keys {
            ...
            utxos.add_key(Loaded { public, private });
        }
        Ok(Core::new(config, utxos))
    }

    pub async fn fetch_utxos(&self) -> Result<()>{
        TcpStream::connect(&self.config.default_node).await?;
        for key in &self.utxos.my_keys {
            ...
            if let Message::UTXOs(utxos) = Message::receive_async(&mut stream).await?{
                ...
                self.utxos.utxos.insert(key.public.clone(), utxos.into_iter().map(|(output, marked)| (marked, output)).collect());
            } else {
    ...
    pub async fn send_transaction (&self, transaction) -> Result<()>{
    ...
    pub fn get_balance(&self) -> u64 {
        self.utxos
           .utxos
           .iter()
           .map(|entry| {entry.value().iter().map(|utxo| utxo.1.value).sum::<u64>()}).sum()
    }
    ...
    pub async fn create_transaction (&self, recipient: &PublicKey, amount: u64) -> Result<Transaction> {
            ...
            for (marked, utxo) in utxos.iter() {
                ...
                inputs.push(btclib::types::TransactionInput {
                    ...
                    signature: btclib::crypto::Signature::sign_output(
                        &utxo.hash(),
                        &self.utxo.my_keys.iter().find(|k| k.public == *pubkey).unwrap().private
                    ),
                });
        ...
        let mut ouputs = vec![TransactionOutput {
            value: amount,
